...
function _M:init()
	...
	self.mhandle_pos = {
		...
		buffs = {x=40 - move_handle[6], y=0, name="正面效果"},--sll 修改buff移动按钮位置
		dbuffs = {x=40 - move_handle[6], y=0, name="负面效果"},
...
function _M:displayPlayerFrame(scale, bx, by)
	...
	if not self.res.pname or self.res.pname.vc ~= player.name then
		self.res.pname = {
			vc = player.name,
			cur = {core.display.drawStringBlendedNewSurface(sfont_sha, player.name, 255, 255, 255):glTexture()},
		}
	end
	local dt = self.res.pname.cur
